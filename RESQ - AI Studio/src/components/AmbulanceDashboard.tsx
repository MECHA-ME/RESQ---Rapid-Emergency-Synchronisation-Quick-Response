                    ...
                    <div key={inc.id} className="bg-white p-6 rounded-2xl shadow-sm border border-brand-border space-y-4">
                      ...
                      <div className="bg-slate-50 p-4 rounded-xl space-y-2 text-xs">
                        ...
                        <div className="text-[11px] text-blue-700 font-semibold">
                          Reporter: {inc.callerRole === 'BYSTANDER' ? `Bystander (${inc.victimCount || 1} victims)` : 'Patient (Self-reported)'}
                        </div>
                      </div>
...
function ActiveMissionView({ incident, state, requestHospitals, selectHospital, updateStatus }: any) {
  const needsHospitals = incident.status === 'RESPONDER_EN_ROUTE';
  const waitingHospitals = incident.status === 'HOSPITAL_COORDINATION';
  const canPickup = incident.status === 'HOSPITAL_SELECTED' || incident.status === 'RESPONDER_EN_ROUTE';
  const canReach = incident.status === 'PATIENT_PICKED' || incident.status === 'IN_TRANSIT';
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-brand-blue">
        ...
        {waitingHospitals && (
          <div className="space-y-4">
            <h3 className="font-bold text-brand-text">Hospital Responses</h3>
            {incident.notifiedHospitals.length > 0 && acceptedHospitalsData.length === 0 && (
              <p className="text-brand-muted animate-pulse text-sm">Waiting for hospitals to accept...</p>
            )}
            
            <div className="space-y-3">
              {acceptedHospitalsData.map((h: User) => (
                <div key={h.id} className="p-4 border border-brand-green bg-green-50 rounded-xl flex items-center justify-between">
                  <div>
                    <p className="font-bold text-brand-green">{h.name}</p>
                    <p className="text-sm text-brand-green">Ready for intake</p>
                  </div>
                  <button 
            ...
            <div className="grid grid-cols-1 grid-cols-1 gap-4">
              <button 
                ...
                onClick={() => {
                  ...
                  setTimeout(() => updateStatus('IN_TRANSIT'), 1000);
                }}
                className={`min-h-[48px] py-4 rounded-xl font-semibold flex flex-col items-center justify-center gap-2 transition-colors ${canPickup ? 'bg-brand-blue hover:opacity-90 text-white' : 'bg-gray-100 text-gray-400 border border-brand-border'}`}
              >
                <CheckCircle size={24} />
                PATIENT PICKED
              </button>
              ...
              <button 
                ...
                className={`min-h-[48px] py-4 rounded-xl font-semibold flex flex-col items-center justify-center gap-2 transition-colors ${canReach ? 'bg-brand-green hover:opacity-90 text-white' : 'bg-gray-100 text-gray-400 border border-brand-border'}`}
              >
                <MapPin size={24} />
                REACHED HOSPITAL
              </button>
