...
export function generateIncidentReportText(incident: Incident, state: AppState): string {
  ...
  const lines: string[] = [
    ...
    `SERVICE TYPE       : ${incident.type === 'AMBULANCE' ? 'AMBULANCE (EMS TRAUMA)' : 'FIRE & RESCUE'}`,
    ...
    `Role / Branch      : ${responder?.role || (incident.type === 'AMBULANCE' ? 'AMBULANCE_DRIVER' : 'FIRE_RESCUE')}`,
...
function renderIncidentToPDF(doc: jsPDF, incident: Incident, state: AppState, startY: number): number {
  ...
  doc.text(incident.type === 'AMBULANCE' ? 'AMBULANCE (EMS)' : 'FIRE & RESCUE', margin + 4, y + 11);
  ...
  const responderDetails = [
    ...
    `• Unit Role / Squad  : ${responder?.role || (incident.type === 'AMBULANCE' ? 'AMBULANCE_DRIVER' : 'FIRE_RESCUE')}`,
...
export function downloadAllHistoryPDF(incidents: Incident[], state: AppState, portalRole?: string) {
  ...
  if (incidents.length === 0) {
    ...
    incidents.slice(0, 15).forEach((inc, idx) => {
      ...
      doc.text(inc.type === 'AMBULANCE' ? 'AMBULANCE' : 'FIRE RESCUE', margin + 42, currentY + 4);
