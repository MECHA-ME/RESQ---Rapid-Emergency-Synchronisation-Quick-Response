...
export default function App() {
  ...
  // Server + database sync monitoring (visible in footer badge)
  const [sync, setSync] = useState({ ok: true, latencyMs: 0, lastOkAt: Date.now(), fails: 0 });
  ...
  const fetchState = async () => {
    ...
    try {
      ...
      setState(data);
      setSync({ ok: true, latencyMs: Math.round(performance.now() - started), lastOkAt: Date.now(), fails: 0 });
    } catch (e) {
      console.error(e);
      setSync(prev => ({ ok: false, latencyMs: prev.latencyMs, lastOkAt: prev.lastOkAt, fails: prev.fails + 1 }));
    }
  };
  ...
  // Push this device's exact GPS to the server as the active role's location
  // (throttled: on move, max once per 10s) so every role tracks for real.
  const { fix: deviceFix } = useLiveGeolocation(isLoggedIn);
  const lastLocPostRef = useRef<{ key: string; at: number }>({ key: '', at: 0 });
  useEffect(() => {
    if (!isLoggedIn || !deviceFix || !userId) return;
    const key = `${deviceFix.lat.toFixed(5)},${deviceFix.lng.toFixed(5)}`;
    const now = Date.now();
    if (key === lastLocPostRef.current.key || now - lastLocPostRef.current.at < 10000) return;
    lastLocPostRef.current = { key, at: now };
    fetch(`/api/users/${userId}/location`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ lat: deviceFix.lat, lng: deviceFix.lng }),
    }).catch(e => console.error('Location sync failed', e));
  }, [isLoggedIn, userId, deviceFix]);
      ...
      >
        ...
        >
          ...
          >
            ...
            {showRoleSwitcher && (
              <div className="absolute inset-0 bg-black/60 backdrop-blur-xs z-[2000] flex flex-col justify-end p-3 animate-fade-in">
            ...
            {/* App Info Modal / View */}
            {showInfoModal && (
              <div className="absolute inset-0 bg-white z-[2000] flex flex-col">
            ...
            {/* Server + Database Sync Detail Panel */}
            {isLoggedIn && showSyncPanel && (
              <div className="absolute inset-x-3 bottom-[68px] z-[1500] bg-slate-900 text-white rounded-2xl border border-slate-700 shadow-2xl p-4 space-y-3 animate-fade-in">
                ...
                <div className="flex items-center justify-between gap-2 bg-white/5 border border-white/10 rounded-xl px-3 py-2">
                  <span className="text-[11px] font-bold text-slate-300">Server</span>
                  <span className={`flex items-center gap-1.5 text-[11px] font-black font-mono ${sync.ok ? 'text-emerald-400' : 'text-red-400'}`}>
                    <span className={`w-2 h-2 rounded-full ${sync.ok ? 'bg-emerald-400 animate-pulse' : 'bg-red-500 animate-ping'}`} />
                    {sync.ok
                      ? `ONLINE${health?.server?.uptimeSec != null ? ` • up ${Math.floor(health.server.uptimeSec / 60)}m ${health.server.uptimeSec % 60}s` : ''}`
                      : 'UNREACHABLE'}
                  </span>
                </div>

                <div className="flex items-center justify-between gap-2 bg-white/5 border border-white/10 rounded-xl px-3 py-2">
                  <span className="text-[11px] font-bold text-slate-300">Database</span>
                  <span className={`flex items-center gap-1.5 text-[11px] font-black font-mono ${sync.ok && health?.database?.connected ? 'text-emerald-400' : 'text-red-400'}`}>
                    <span className={`w-2 h-2 rounded-full ${sync.ok && health?.database?.connected ? 'bg-emerald-400 animate-pulse' : 'bg-red-500 animate-ping'}`} />
                    {sync.ok && health?.database?.connected
                      ? `CONNECTED • ${health.database.mode} • ${health.database.incidents} incidents`
                      : health?.database ? 'REPORTED ERROR' : 'UNKNOWN (no health reply)'}
                  </span>
                </div>

                <div className="grid grid-cols-3 gap-2 text-center">
                  <div className="bg-white/5 border border-white/10 rounded-xl px-2 py-1.5">
                    <div className="text-[9px] font-bold uppercase text-slate-400">Latency</div>
                    <div className="text-xs font-black font-mono text-white">{sync.ok ? `${sync.latencyMs}ms` : '—'}</div>
                  </div>
                  <div className="bg-white/5 border border-white/10 rounded-xl px-2 py-1.5">
                    <div className="text-[9px] font-bold uppercase text-slate-400">Last sync</div>
                    <div className="text-xs font-black font-mono text-white">{Math.floor((Date.now() - sync.lastOkAt) / 1000)}s ago</div>
                  </div>
                  <div className="bg-white/5 border border-white/10 rounded-xl px-2 py-1.5">
                    <div className="text-[9px] font-bold uppercase text-slate-400">Records</div>
                    <div className="text-xs font-black font-mono text-white">{state.incidents.length + state.feedbacks.length}</div>
                  </div>
                </div>

                {!sync.ok && (
                  <p className="text-[11px] text-red-300 leading-snug">
                    Cannot reach the server. Check that `npm run dev` is running and port 3000 is free, then the app will auto-reconnect.
                  </p>
                )}
              </div>
            )}
            ...
            {isLoggedIn && (
              <footer className="bg-white border-t border-gray-200/90 px-4 py-2.5 flex items-center justify-between shrink-0 z-30 shadow-md">
                ...
                {/* Center: Live Sync Status Badge (tap for server + DB detail) */}
                <button
                  onClick={() => { fetchHealth(); setShowSyncPanel(v => !v); }}
                  title="Server & database sync status — tap for details"
                  className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[10px] font-black font-mono transition-all cursor-pointer active:scale-95 ${
                    sync.ok
                      ? 'bg-emerald-50 border-emerald-200 text-emerald-700'
                      : 'bg-red-50 border-red-300 text-red-700 animate-pulse'
                  }`}
                >
                  <span className={`w-1.5 h-1.5 rounded-full ${sync.ok ? 'bg-emerald-500 animate-pulse' : 'bg-red-500 animate-ping'}`} />
                  <span>{sync.ok ? `SYNC ACTIVE • ${sync.latencyMs}ms` : `OFFLINE • RETRYING${sync.fails > 1 ? ` (${sync.fails})` : ''}`}</span>
                </button>
